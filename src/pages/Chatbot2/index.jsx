...
import { ChatQuoteFill, PersonUp, PlusLg, Trash, XLg, SendFill, HandThumbsUpFill, HandThumbsDown } from 'react-bootstrap-icons';
...
function Chatbot2() {
  ...
  useEffect(() => {
    ...
    if (id == undefined || null || "") {
      navigate(`/chatbot-s2?id=${chatbotData[0].id}`)
    }
        ...
        <SimpleBar className="tyn-aside-body">
            <ul className="tyn-aside-list">
              {chatbotData.map((item,index)=>{
                return(
                  <li key={index} className={classNames({"tyn-aside-item tyn-aside-item-bubbly":true, "active":item.id == selected?.id})} 
                    onClick={(ev) => {
                        ev.preventDefault();
                        if(!ev.target.closest('.tyn-aside-item-option')){
                            navigate(`/chatbot-s2?id=${item.id}`);
                            setShowMain(true);
                        }
                    }}>
                    <Media.Group>
                    ...
                    {selected?.chats?.map((item,index)=> {
                        return(
                            <div key={index} className="tyn-qa-item">
                                {item.who === "bot" && 
                                    <ul className="position-absolute d-flex mb-n3 me-3 end-0 bottom-0">
                                        <li className="px-1 my-1">
                                            <Button variant="white" size="md" className="btn-icon btn-pill shadow">
                                                <HandThumbsUpFill />
                                            </Button>
                                        </li>
                                        <li className="px-1 my-1">
